...
# Define prize type and number of top annotations
prize_type <- "Total_Medals"  # This is now for all medals
top_n_numbers <- 5  # Number of top countries to annotate

# Calculate total medals (sum of gold, silver, and bronze)
data <- data %>%
...
# Extract the total medal counts and their corresponding GDP values
...
plot_title <- paste("Total Medals vs GDP")
...
plot <- ggplot(top_data, aes(x = Total_Medals, y = GDP)) +
  ...
  theme(
    ...
    axis.text = element_text(color = "black", size = 12),  # Black bold axis text
    axis.title = element_text(color = "black", size = 14), # Black bold axis labels
